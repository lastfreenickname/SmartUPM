...
public class SimpleApplet extends javacard.framework.Applet
{
    ...
    final static byte  AES_BLOCK_LENGTH               = (short) 0x10;
    ...
    void SetKey(APDU apdu) {
      ...
      // INIT CIPHERS WITH NEW KEY
      // separate objects for enc and dec - because init (change mode) takes long time (10ms)
      m_encryptCipher.init(m_aesKey, Cipher.MODE_ENCRYPT);
      m_decryptCipher.init(m_aesKey, Cipher.MODE_DECRYPT);
    }
    // ENCRYPT INCOMING BUFFER
     void Encrypt(APDU apdu) {
      ...
      // CHECK EXPECTED LENGTH (MULTIPLY OF 128 bits - AES block size)
      if ((dataLen % 16) != 0) ISOException.throwIt(SW_CIPHER_DATA_LENGTH_BAD);

      // ENCRYPT INCOMING BUFFER
      // writing output to RAM. Could write also directly to the same buffer (apdubuf), with
      // offset 0 - so that I don't overwrite the unencrypted plaintext
    ...
    void Decrypt(APDU apdu) {
      ...
      // CHECK EXPECTED LENGTH (MULTIPLY OF 128 bits)
      if ((dataLen % 16) != 0) ISOException.throwIt(SW_CIPHER_DATA_LENGTH_BAD);
    ...
    // HASH INCOMING BUFFER
     void Hash(APDU apdu) {
      ...
      if (m_hash != null) {
          ...
          // .doFinal takes all data and creates final hash. 
          // m_hash.update would work when data arrives block by block
      }
