   ...
   private static     byte[] key = {(byte) 0x00, (byte) 0x01, (byte) 0x02, (byte) 0x03, (byte) 0x04, (byte) 0x05, (byte) 0x06,(byte) 0x07,(byte) 0x08, (byte) 0x09, (byte) 0x0A, (byte) 0x0B, (byte) 0x0C, (byte) 0x0D, (byte) 0x0E,(byte) 0x0F};
    ...
    public void process(APDU apdu) throws ISOException
    {
        ...
        if (apduBuffer[ISO7816.OFFSET_CLA] == CLA_HEADER) {
            ...
            {
                case INS_SETKEY: 
                    if (NumKey<DB_CNT) SetKey(apdu); 
                    else ISOException.throwIt(SW_DB_COUNT_EXCEEDED);
                    break;
    ...
    // VERIFY PIN
     void GetKey(APDU apdu) {
      ...
      if(PINArray[DBID].getTriesRemaining()==1)
      {
        ISOException.throwIt(SW_BAD_PIN_LASTATTEMPT);
      }      
      if(PINArray[DBID].getTriesRemaining()==0)
      {
        ...
        ISOException.throwIt(SW_DB_PERMANENTLY_LOCKED);
      }
      else
